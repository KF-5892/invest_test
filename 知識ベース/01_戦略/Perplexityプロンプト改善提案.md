...
## 問題1: 「過去の調査結果を参照」が差分管理として不十分

**ファクトチェック補足（2026-02-26）:**
Perplexityは2025年4月にメモリ機能をベータ開始し、**2025年11月に全ユーザーへ正式展開**している。セッションをまたいだ記憶は技術的に可能であり、「Perplexityは毎回新規セッションで起動するため参照できない」という当初の前提は現時点では不正確。

ただし、メモリ機能に依存するだけでは差分管理として不十分な理由が3点ある：
1. **記憶粒度が粗い** — 「投資・システムトレードに興味がある」程度の要約が保存されるため、「モメンタム戦略を2/14に調査済み」という構造化された追跡はしてくれない
2. **能動的な更新ができない** — 自動生成メモリは「どの戦略まで調査済みか」を正確に管理しない
3. **信頼性に限界あり** — 削除したメモリが復活する不具合の報告がある

実際に2/14と2/25の結果を比べると、メモリ機能が有効であっても「AIエージェント量産」「DRL進化」「モメンタム」という同じトレンドが繰り返されており、差分が出ていない。

**改善案:**
メモリ機能の有無にかかわらず、前回結果のサマリー（既知の戦略リスト）をプロンプト冒頭に明示的に渡す。これにより自動メモリより確実に差分管理できる。「以下の内容は把握済みです。**新しい情報のみ**を提供してください」と指定する。
