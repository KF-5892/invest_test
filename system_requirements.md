...
|---------|--------|------------|---------|
| **立花証券e支店API** | Linux/Docker可 | GCP/AWS対応 | API仕様が特殊だがクラウドデプロイ可能。**Linux環境では最有力** |
| **カブコム証券API** | Windows必須 | Windows VPS | Kabuステーション経由。デイトレ信用金利0%。2025年6月～メール2段階認証必須 |
| **楽天RSS** | Windows必須 | Windows VPS | マーケットスピード起動必須 |
| **岡三RSS** | Windows必須 | Windows VPS | 岡三ネットトレーダー起動必須 |
| **UFJeスマート証券API** | Linux/Docker可 | GCP/AWS対応 | REST API。比較的新しいサービス |

### 推奨構成

- **Linux/Docker環境を前提とする場合**: 立花証券e支店API
- **Windows環境を許容する場合**: カブコム証券API（デイトレ信用金利0%のメリット）
...
| 証券会社 | 認証方式 | 自動化手法 |
|---------|---------|-----------|
| カブコム | メール認証（2025年6月～） | IMAP受信 → 正規表現で認証コード抽出 → 自動入力。Gmail API or S3転送も可。メール到着が5分の有効期限内に来ないリスクあり |
| 立花証券 | 電話番号認証（2025年7月末～） | Twilio (050番号) → Python SIPで自動発信 |
...
- **Python 3.10以上**を推奨（match文、型ヒント改善等）
